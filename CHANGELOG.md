...
- Session Handoff as a first-class Workspace section + MCP tools. The full kit is tracked at `vendor/session-handoff-kit/` (Claude Code plugin, Cowork/chat artifacts via `package.sh`, Codex installer, Chrome extension, ledger CLI). UI Add / manage per README surface (copy `/plugin` commands, `package.sh`, Codex `install.sh`, Chrome path). MCP: `handoff_status`, `handoff_skill`, `handoff_template`, `handoff_list`, `handoff_resolve`, `handoff_new_path`, `handoff_resume`, `handoff_supersede`, `handoff_install_help`. Ledger writes only via the vendored `handoff_ledger.py`. `doctor` reports kit present/configured (fail-closed on a bad override). Optional override: `settings.handoff_kit` / `PORTSKILL_HANDOFF_KIT`.
