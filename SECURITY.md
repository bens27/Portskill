...
- **No OAuth / passkeys / friend-installer / signed-app distribution.** `http_auth.json` may exist as an optional helper; it does not gate default UI/API routes. Non-loopback `--host` (e.g. `0.0.0.0`) is **refused at start** unless `--allow-non-loopback` is passed (documented footgun). `doctor` fails closed (exit 2) when `listen.json` shows a non-loopback bind without that override recorded. With the override, start is allowed, the UI banner/chip stays, and `doctor` warns but exits 0. Default bind stays loopback.
